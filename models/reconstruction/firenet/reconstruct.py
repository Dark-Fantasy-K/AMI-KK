#!/usr/bin/env python3
"""
FireNet reconstruction from a single events file (.npy or .txt).
...
Usage:
    python models/reconstruction/firenet/reconstruct.py \
        --events_file data/events.npy \
        --weights     models/reconstruction/firenet/pretrained_weights/firenet.pth \
...
from models.reconstruction.firenet.model import load_firenet
...
def reconstruct(
    ...
    """
    Run FireNet frame-by-frame on events dict.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="FireNet reconstruction from events file")
    p.add_argument("--events_file",      required=True, help="Path to .npy or .txt events file")
    p.add_argument("--weights",          required=True, help="Path to FireNet .pth checkpoint")
