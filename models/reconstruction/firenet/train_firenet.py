#!/usr/bin/env python3
"""
train_firenet.py — Train (or fine-tune) FireNet on the FRED dataset.

Architecture: head → G1 → G2 → G3 → pred  (all at full resolution)
...
Usage — fine-tune from official FireNet checkpoint:
    python models/reconstruction/method2/train_firenet.py \\
        --fred_root  /data/FRED \\
        --weights    models/reconstruction/method2/pretrained_weights/firenet.pth \\
        --save_dir   checkpoints/firenet \\
        --epochs     50
...
Resume training:
    python models/reconstruction/method2/train_firenet.py ... --resume checkpoints/firenet/last.pth
"""
...
from models.reconstruction.method2.model import FireNet, load_firenet
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Train FireNet on the FRED event-camera dataset",
    ...
    # Model
    p.add_argument("--weights",  default=None, help="Pretrained FireNet checkpoint for fine-tuning")
    p.add_argument("--resume",   default=None, help="Resume from our checkpoint (last.pth)")
    # Training
    p.add_argument("--save_dir",     default="checkpoints/firenet")
...
def main() -> None:
    ...
    if args.weights:
        ...
        model = FireNet(num_bins=args.num_bins).to(device)
    ...
    criterion = FireNetLoss(args.lambda_l1, args.lambda_ssim, args.lambda_tv)
